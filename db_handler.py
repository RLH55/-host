...
class MongoDBHandler:
    ...
    def delete_server(self, server_id):
        """حذف سيرفر محدد من قاعدة البيانات نهائياً"""
        if not self.connected:
            return self._delete_server_local(server_id)
        try:
            self.db['servers'].delete_one({"_id": server_id})
            print(f"✅ تم حذف السيرفر {server_id} من MongoDB")
            return True
        except Exception as e:
            print(f"❌ خطأ في حذف السيرفر من MongoDB: {e}")
            return self._delete_server_local(server_id)

    def delete_user(self, username):
        """حذف مستخدم محدد من قاعدة البيانات نهائياً"""
        if not self.connected:
            return self._delete_user_local(username)
        try:
            self.db['users'].delete_one({"_id": username})
            print(f"✅ تم حذف المستخدم {username} من MongoDB")
            return True
        except Exception as e:
            print(f"❌ خطأ في حذف المستخدم من MongoDB: {e}")
            return self._delete_user_local(username)

    def _delete_server_local(self, server_id):
    ...
    def _delete_user_local(self, username):
        db_file = os.path.join(os.path.dirname(__file__), "db.json")
        try:
            if os.path.exists(db_file):
                with open(db_file, 'r', encoding='utf-8') as f:
